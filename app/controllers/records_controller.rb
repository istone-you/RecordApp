...
def all_index
  ...
  private
    ...
    def record_params
      params.require(:record).permit(:user_id, :title, :count, :goal_count, :coment, :image, :money, :done, :minutes, :hours, :link, :created_at, :youtube, :twitter, :start_time, tag2_ids: []).merge(folder_id: params[:folder_id],user_id: current_user.id)
    end
